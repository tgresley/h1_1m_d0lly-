const CURSORS = {
  base:  "url('./2015'), auto",
  click: "url('./chungus'), pointer",
};

///// Coarse source text (paste yours in full)
let sourceText = `
  lip
...
function setup() {
  ...
  // Base cursor applied to canvas & body
  document.body.style.cursor = CURSORS.base;
  // p5 creates a <canvas> element; grab it and set cursor too
  const cnv = document.querySelector('canvas');
  if (cnv) cnv.style.cursor = CURSORS.base;
  ...
  injectCentralLink();  // puts the central link element into DOM
}

function draw() {
  background(255);
  displaySentences();        // render with per-char wave + 15% blue
  highlightWordUnderMouse(); // hover blue + underline + cursor swap
  maybeRevealCentralLink();  // show central link once enough lines exist
}
...
///// Parse source text into sentence fragments
...
function createCoherentSentence() {
  ...
  if (random() < 0.5 && sentences.length) {
    ...
    sentence = `${capitalizeFirstLetter(random(subjects))} ${random(verbs)} ${random(objects)}.`;
  }
...
///// Style to produce wave & random blue chars
...
///// Words list (used for categorization + spawning)
...
///// Hover detection + cursor swap
function highlightWordUnderMouse() {
  highlightedWord = null;

  // default cursor each frame (base)
  const cnv = document.querySelector('canvas');
  if (cnv) cnv.style.cursor = CURSORS.base;
  ...
  for (let pIndex = 0; pIndex < sentenceParticles.length; pIndex++) {
    ...
    for (let wIndex = 0; wIndex < words.length; wIndex++) {
      ...
      if (mouseX >= left && mouseX <= right && mouseY >= top && mouseY <= bottom) {
        ...
        // swap to clickable cursor on hover
        if (cnv) cnv.style.cursor = CURSORS.click;
        return;
      }
...
///// Render sentence blocks with per-char style & hover underline
  ...
  for (let pIndex = 0; pIndex < sentenceParticles.length; pIndex++) {
    ...
    for (let wIndex = 0; wIndex < words.length; wIndex++) {
      ...
      for (let i = 0; i < token.length; i++) {
        ...
        fill(isHover ? color(0, 102, 204) : styled.color);
      ...
      if (isHover) {
        ...
        stroke(0, 102, 204);
        strokeWeight(Math.max(1, size * 0.06));
...
function findNonOverlappingPosition() {
  ...
  const newW = Math.min(440, width * 0.8);
  const newH = 120;
  const pad = 22;
...
function findNonOverlappingPositionNear(tx, ty){
  ...
  const newW = Math.min(440, width*0.8), newH = 120, pad = 22;
...
///// Central link reveal after enough wrapped lines
const CENTRAL_LINK_ID = 'nextLink';
const LINE_THRESHOLD  = 7;

function injectCentralLink(){
  // Ensure a super-high stacking context + always clickable
  const css = `
    #${CENTRAL_LINK_ID}{
      ...
      cursor: ${CURSORS.click};
    }
    #${CENTRAL_LINK_ID}.show{
  ...
  linkEl.href = './smiley.html';
  linkEl.textContent = 'ʕ •ᴥ•ʔ';

  // Extra safety: navigate via JS too
  linkEl.addEventListener('click', (e) => {
    e.preventDefault();
    // If the file exists, this will work. If not, you’ll get a 404 (so check the path).
    window.location.assign(linkEl.href);
  });

  document.body.appendChild(linkEl);

  // Debug: press "L" to toggle visibility if you want to test quickly
  document.addEventListener('keydown', (e) => {
    if (e.key.toLowerCase() === 'l') {
      linkEl.classList.toggle('show');
    }
  });
}
